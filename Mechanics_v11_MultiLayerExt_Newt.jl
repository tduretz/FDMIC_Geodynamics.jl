...
##############
function SetMarkers!( p, params, dom )    # Use this function to set up the model geometry
    R  = params.user[1]
    L  = dom.xmax - dom.xmin
    H  = dom.ymax - dom.ymin
    x1 = -1.0
    ...
    for k=1:p.nmark #@tturbo <---------rand does not work in @tturbo!!!!
        ...
        layer1     = abs(p.y[k] + is_pert*(dom.dy*rand()-dom.dy/2) - 0.2*H) < R/3.0
        layer2     = abs(p.y[k] + is_pert*(dom.dy*rand()-dom.dy/2) - 0.1*H) < R/3.0
        layer3     = abs(p.y[k] + is_pert*(dom.dy*rand()-dom.dy/2) - 0.00000*H) < R/3.0
        layer4     = abs(p.y[k] + is_pert*(dom.dy*rand()-dom.dy/2) + 0.1*H) < R/3.0
        layer5     = abs(p.y[k] + is_pert*(dom.dy*rand()-dom.dy/2) + 0.2*H) < R/3.0
...
@views function main( )
    ...
    for it=1:params.nt
        ...
        if params.show_figs==1 && ( mod( it, params.nout )==0 || it==1 )
            p1 = Plots.heatmap(domain.xc,  domain.yc, Array(Tau.II)', aspect_ratio=1, xlims=(domain.xmin, domain.xmax), ylims=(domain.ymin, domain.ymax), c=Plots.cgrad(:roma, rev = true), title="Tii")
            display(Plots.plot( p1, dpi=200 ) ); Plots.frame(anim) 
        end
    end
