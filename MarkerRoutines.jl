...
@views function FindClosestNeighbour( p, mpc, mlist, new_x, new_y, dxy, i, j, ncx, ncy )
    ineigh = 0
    dst = dxy
    # Current cell
...
@views function ReseedMarkers!( p, mpc, xc, yc, dx, dy, ncx, ncy)
...
@views function CountMarkers2!(p,mpc,mpc_th,phase_perc,phase_perc_th,nphase,nmark,xmin,xmax,ymin,ymax,xc,yc,dx,dy,ncx,ncy)
...
@views function Markers2Cells3!(p,phase,xc,yc,dx,dy,ncx,ncy,prop,avg,mode)
...
@views function CountMarkers!(p,mpc,mpc_th,nmark,xmin,xmax,ymin,ymax,xc,yc,dx,dy,ncx,ncy)
    # Disable markers outside of the domain
    @threads for k=1:nmark
        if (p.x[k]<xmin || p.x[k]>xmax || p.y[k]<ymin || p.y[k]>ymax) 
            @inbounds p.phase[k] = -1
