...
@views function main( )
    ...
    params.Ebg       = -0.0e-18      # reference strain rate (>0 horizontal compression)
    # Scales
    scale            = CharScales()
    scale.L          = 1e3
    scale.T          = 1.0
    scale.t          = 1e18#1.0/abs(params.Ebg)  
    scale.S          = 1e8   
    scale.m          = scale.S*scale.L*scale.t^2 
    scale.rho        = scale.S*scale.L^(-2)*scale.t^2
    # Spatial discretisation
    ncx              = 150;  domain.ncx = ncx
    ncy              = 151;  domain.ncy = ncy
    # Time discretisation
    params.advection = 0
    params.nt        = 1#2000
    params.dt        = 1.25e7
    ...
    params.niter_nl  = 20    # max. number of non-linear iterations
    ...
    params.gy    = -9.811   / (scale.L/scale.t^2)
    ...
    fields.phiDam .= 0.0
    fields.Pc  .= -reverse(cumsum(fields.rhoc.*params.gy*dy, dims=2), dims=2)
    fields.Pc .+= 0.5*fields.rhoc.*params.gy*dy
#     p3 = Plots.heatmap( domain.xc, domain.yc, Array(fields.Pc*scale.S./1e6)', aspect_ratio=1, xlims=(domain.xmin, domain.xmax), ylims=(domain.ymin, domain.ymax), c=Plots.cgrad(:roma, rev = true), title="Tii" )
# display(p3)
#     error("")
    ...
    for it=1:params.nt
        ...
        if params.show_figs==1 && ( mod( it, params.nout )==0 || it==1 )
            l = @layout [ [a; b] c]
            cmy = 3600*24*365*100
            p1 = Plots.heatmap( domain.xc, domain.yc, (fields.rhoc*scale.rho)', aspect_ratio=1, xlims=(domain.xmin, domain.xmax), ylims=(domain.ymin, domain.ymax), c=Plots.cgrad(:roma, rev = true), title="rho [kg/m^3]",  clim=(2500,2700)  )
            ...
            # p1 = Plots.heatmap( domain.xc, domain.yc, Array(fields.phiDam)', aspect_ratio=1, xlims=(domain.xmin, domain.xmax), ylims=(domain.ymin, domain.ymax), c=Plots.cgrad(:roma, rev = true), title="phi, niter = $iterations",  clim=(0.0,1.0) )
            ...
            p3 = Plots.heatmap( domain.xc, domain.yc, Array(fields.Pc0)'./1e6*scale.S, aspect_ratio=1, xlims=(domain.xmin, domain.xmax), ylims=(domain.ymin, domain.ymax), c=Plots.cgrad(:roma, rev = true), title="P0 [MPa], nt = $it" )
            p4 = Plots.heatmap( domain.xc, domain.yc, Array(fields.Pc)'./1e6*scale.S, aspect_ratio=1, xlims=(domain.xmin, domain.xmax), ylims=(domain.ymin, domain.ymax), c=Plots.cgrad(:roma, rev = true), title="P [MPa], nt = $it" )
