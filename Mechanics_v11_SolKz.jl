...
@views function main( N )
    ...
    for j=1:ncy+2
        vx,vy,P,ρ,η =  _solkz_solution(domain.xmin, domain.yce[j])
        BC.Vx.Dir_W[j] = vx
        vx,vy,P,ρ,η =  _solkz_solution(domain.xmax, domain.yce[j])
        BC.Vx.Dir_E[j] = vx
    end
    # Evaluate BC's: South/North Vx
    for i=1:ncx+1
        vx,vy,P,ρ,η =  _solkz_solution(domain.xv[i], domain.ymin)
        BC.Vx.Dir_S[i] = vx
        vx,vy,P,ρ,η =  _solkz_solution(domain.xv[i], domain.ymax)
        BC.Vx.Dir_N[i] = vx
    end
    # Evaluate BC's: West/East Vy
    for j=1:ncy+1
        vx,vy,P,ρ,η =  _solkz_solution(domain.xmin, domain.yv[j])
        BC.Vy.Dir_W[j] = vy
        vx,vy,P,ρ,η =  _solkz_solution(domain.xmax, domain.yv[j])
        BC.Vy.Dir_E[j] = vy
    end
    # Evaluate BC's: South/North Vy
    for i=1:ncx+2
        vx,vy,P,ρ,η =  _solkz_solution(domain.xce[i], domain.ymin)
        BC.Vy.Dir_S[i] = vy
        vx,vy,P,ρ,η =  _solkz_solution(domain.xce[i], domain.ymax)
        BC.Vy.Dir_N[i] = vy
    end
    # Allocate tables
    fields           = Fields2D()
    ...
    CentroidsToVertices!( fields.etav, fields.etac, ncx, ncy, BC )
    # Set pinned pressure
    ...
    if params.show_figs==1
        p1 = Plots.heatmap(domain.xc, domain.yc, Array(Vxc)', aspect_ratio=1, xlims=(domain.xmin, domain.xmax), ylims=(domain.ymin, domain.ymax), c=Plots.cgrad(:lajolla, rev = true), title="Vx")
        p2 = Plots.heatmap(domain.xc, domain.yc, Array(Vxca)', aspect_ratio=1, xlims=(domain.xmin, domain.xmax), ylims=(domain.ymin, domain.ymax), c=Plots.cgrad(:lajolla, rev = true), title="Vxa")

        # p2 = Plots.heatmap(domain.xc, domain.yc, Array(Vyc)', aspect_ratio=1, xlims=(domain.xmin, domain.xmax), ylims=(domain.ymin, domain.ymax), c=Plots.cgrad(:lajolla, rev = true), title="Vy")
        p3 = Plots.heatmap(domain.xc, domain.yc, Array(fields.Pc)',        aspect_ratio=1, xlims=(domain.xmin, domain.xmax), ylims=(domain.ymin, domain.ymax), c=Plots.cgrad(:lajolla, rev = true), title="P")
        # p4 = Plots.heatmap(domain.xv, domain.yv, Array(fields.rhoc)', aspect_ratio=1, xlims=(domain.xmin, domain.xmax), ylims=(domain.ymin, domain.ymax), c=Plots.cgrad(:lajolla, rev = true), title="ρ")
        p4 = Plots.heatmap(domain.xv, domain.yv, Array(Pca)', aspect_ratio=1, xlims=(domain.xmin, domain.xmax), ylims=(domain.ymin, domain.ymax), c=Plots.cgrad(:lajolla, rev = true), title="Pa")
